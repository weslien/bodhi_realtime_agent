...
// src/memory/memory-distiller.ts
var factsSchema = import_zod3.z.object({
...
var ElevenLabsSTTProvider = class {
  ...
  async stop() {
    ...
    if (this._ws) {
      if (this._ws.readyState === import_ws2.WebSocket.OPEN) {
  ...
  feedAudio(base64Pcm) {
    ...
    if (this._state === "connected" && this._ws?.readyState === import_ws2.WebSocket.OPEN) {
      this._send({
  ...
  _connect() {
    return new Promise((resolve, reject) => {
      ...
      this._ws = new import_ws2.WebSocket(url.toString(), {
...
var import_ws3 = require("openai/realtime/ws");
...
var OpenAIRealtimeTransport = class {
  ...
  async connect(transportConfig) {
    ...
    this.rt = await import_ws3.OpenAIRealtimeWS.create(this.client, { model });
